...
**Rationale:** `mocker` ties patch lifecycle to pytest fixtures (no leaked patches across tests). `unittest.mock` decorators/context managers are easy to mis-compose with parametrized or async tests and are inconsistent with in-tree L1 style.
