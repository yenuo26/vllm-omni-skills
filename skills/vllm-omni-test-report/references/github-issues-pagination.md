...
## Filter by stats window (`created_at` UTC date)

Reports generated by **`compose_full_report.py`** (and **`patch_report_open_issues.py`**) keep only open **`label:bug`** issues (PRs excluded) whose **`created_at`** calendar date in **UTC** falls in **`--stats-from` … `--stats-to`** (inclusive `YYYY-MM-DD`), same as **Metrics overview** / **Analysis (CI Failure)**.

Example: window `2026-04-01` … `2026-05-07`:

```bash
jq --arg from "2026-04-01" --arg to "2026-05-07" \
  '[.[] | select(.created_at[0:10] >= $from and .created_at[0:10] <= $to)]'
```
