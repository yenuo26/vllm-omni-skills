...
2. **Metrics overview** — `buildkite_build_stats.py --markdown` (**Success rate/UT coverage** per ready/merge/nightly/weekly bucket; **ready per-PR avg success rate** = mean of each PR’s CI success rate in the **ready** bucket, distinct from the pooled **ready** row; **Bug avg first response**; **ut** / **ut (exclude models)**，与 **`--stats-from`..`--stats-to`** 对齐)。
...
3. Add `--markdown` to print a ready-to-paste **Metrics overview** block: Source line plus CI category table (**Success rate/UT coverage** for ready/merge/nightly/weekly; **ready per-PR avg success rate** = mean of per-PR success rates in the **ready** bucket, not the pooled **ready** row; **Bug avg first response** on **bugs (first response, YYYY-MM-DD..YYYY-MM-DD)** row from GitHub - same date window as **`--from` / `--to`**; **ut** / **ut (exclude models)** from **Simple Unit Test** log parsing - implementation detail stays in `buildkite_build_stats.py`, not in the pasted report prose).
